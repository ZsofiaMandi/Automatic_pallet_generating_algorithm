...
def mirroring_layer(input_box_list, x, y, pallet_x, pallet_y, box_x, box_y, middle=False):

    input_box_list_mirrored = copy.deepcopy(input_box_list)

    # finding the las box coordinates in the list:
    max_index = 0
    min_index = 0
    max_coordinate = 0
    min_coordinate = input_box_list[0][0] + input_box_list[0][1]
    i = 0
    for box in input_box_list_mirrored:
    ...
    if input_box_list_mirrored[max_index][3] == 0:
        max_x = box_x
        max_y = box_y
    elif input_box_list_mirrored[max_index][3] == 1:
        max_x = box_y
        max_y = box_x

    # Checking the first box x and y dimensions
    if input_box_list_mirrored[min_index][3] == 0:
        min_x = box_x
        min_y = box_y
    elif input_box_list_mirrored[min_index][3] == 1:
        min_x = box_y
        min_y = box_x

    # Calculating the loading of the pallet
    load_x_max = input_box_list_mirrored[max_index][0] + max_x / 2
    load_y_max = input_box_list_mirrored[max_index][1] + max_y / 2
    load_x_min = input_box_list_mirrored[min_index][0] - min_x / 2
    load_y_min = input_box_list_mirrored[min_index][1] - min_y / 2
    ...
    return input_box_list_mirrored
