...
def labeling(output_box_list, label_side, label_place, box_x, box_y):

    box_list_labeled = copy.deepcopy(output_box_list)
    ...
    if min_index_0_x == max_index_0_x:
        if box_list_labeled[min_index_0_x][0] < pallet_load_x / 2:
            max_index_0_x = -1
        else:
            min_index_0_x = -1

    if min_index_1_x == max_index_1_x:
        if box_list_labeled[min_index_1_x][0] < pallet_load_x / 2:
            max_index_1_x = -1
        else:
            min_index_1_x = -1

    if min_index_0_y == max_index_0_y:
        if box_list_labeled[min_index_0_y][0] < pallet_load_y / 2:
            max_index_0_y = -1
        else:
            min_index_0_y = -1

    if min_index_1_y == max_index_1_y:
        if box_list_labeled[min_index_1_y][0] < pallet_load_y / 2:
            max_index_1_y = -1
        else:
            min_index_1_y = -1
    ...
    if label_side == "Front":
        for box in box_list_labeled:
            ...
            # If box orientation is 1 -> rotate with 180° to orientation 3, if 0 -> all labels will be on the front side
                ...
                if min_index_0_y >= 0:
                    if box[3] == 0 and box[1] == box_list_labeled[min_index_0_y][1]:    # Front
                        pass
                if min_index_1_x >= 0:
                    if box[3] == 1 and box[0] == box_list_labeled[min_index_1_x][0]:    # Left
                        box[3] = 3
                if max_index_0_y >= 0:
                    if box[3] == 0 and box[1] == box_list_labeled[max_index_0_y][1]:    # Back
                        box[3] = 2
                if max_index_1_x >= 0:
                    if box[3] == 1 and box[0] == box_list_labeled[max_index_1_x][0]:    # Right
                        pass
    ...
    # The label is on the Back side of the box in the beginning (along with x, the down side from the top view)
        ...
        for box in box_list_labeled:
            ...
            # If box orientation is 0 -> rotate with 180° to orientation 2, if 1 -> all labels will be on the front side
                ...
                if min_index_1_y >= 0:
                    if box[3] == 1 and box[1] == box_list_labeled[min_index_1_y][1]:    # Front
                        pass
                if min_index_0_x >= 0:
                    if box[3] == 0 and box[0] == box_list_labeled[min_index_0_x][0]:    # Left
                        pass
                if max_index_1_y >= 0:
                    if box[3] == 1 and box[1] == box_list_labeled[max_index_1_y][1]:    # Back
                        box[3] = 3
                if max_index_0_x >= 0:
                    if box[3] == 0 and box[0] == box_list_labeled[max_index_0_x][0]:    # Right
                        box[3] = 2
